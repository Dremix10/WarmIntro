    ...
    for (const msg of inbox) {
      ...
      await sendTelegram(
        `📬 Alma reply received\n\n` +
          ...
          `Next stage: ${classification.nextStage ?? "(none)"}\n` +
          `Connection: ${match.connectionId ?? "(none)"}\n` +
          `Draft: ${match.draftId ?? "(none)"}\n\n` +
          `Preview:\n${msg.body.slice(0, 700)}`
      );
...
Return JSON: {"sendTime"?: "HH:MM", "extraDrafts"?: number, "skipDay"?: boolean, "trustLevel"?: "C"|"B"|"A"}. Omit fields the reply doesn't address.`,
  ...
  if (Object.keys(override).length > 0) {
    // Stamp the override with an expiry so it can't get stuck forever. The
    // intent of "tomorrow_override" is one day's worth of behavior. 36h
    // covers the next day's batch comfortably regardless of when the user
    // replied (night-preview goes at ~9pm user-local; replies up to ~18h
    // later still land before the next day's send window). Tick handler
    // auto-clears expired rows.
    override.validUntil = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
    await restUpdate(
      "trust_levels",
