...
export function FunnelMathAnimated() {
  ...
  return (
    ...
    >
      {/* Mobile: vertical funnel staircase. Each stage is a horizontal row
          with a count, label, and a bar that shortens top-to-bottom — a
          literal funnel shape. The 7-cramped-bars design crashes on a 390px
          screen; this reads cleaner. */}
      ...
      {/* Desktop: original cascading-bars chart */}
...
/**
 * Mobile-only funnel row. Reads as: COUNT [bar] LABEL
 * Bar width = (value / MAX) * available — the 1-offer row shows as a sliver.
 * Animates with a delayed slide-in via the same `state` prop.
 */
function FunnelRowMobile({
  ...
  return (
    <div className="flex items-center gap-2.5">
      <span
        className={`shrink-0 w-9 text-right font-[family-name:var(--font-fraunces)] tabular-nums leading-none ${
          isLast ? "text-2xl" : "text-xl"
        } ${numColor}`}
      >
        {count}
      </span>
      <div className="flex-1 h-3.5 rounded-md bg-[#F4EDDB] overflow-hidden">
        <div
      ...
      <span className="shrink-0 w-[88px] text-[10px] uppercase tracking-[0.08em] text-[#14182A]/80 font-medium leading-tight">
        {stage.label}
      </span>
    </div>
  );
