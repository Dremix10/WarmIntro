...
export async function GET(request: Request) {
  ...
  const { data: drafts } = await ctx.supabase
    .from("drafts")
    .select("id, banker_id, type, subject, body, status, iteration_count, scheduled_send_at, created_at, fact_check, bankers(name, title, email, linkedin_url, firm_id, firms(name))")
