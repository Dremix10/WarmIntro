...
// Tick window must be ≥ the cron cadence, otherwise users whose
// preferred_send_time doesn't land exactly on a tick mark never fire. The
// Ubuntu VPS hits this endpoint every 30 min on the half-hour, so a 30-min
// window plus the 30-min dedup gives every user exactly one planner run per
// day in the bin containing their send time. Observed bug: at window=1 only
// users whose send_time was 07:00 (= 11:00 UTC, exact cron mark) ever fired;
// Anya at 13:35 ET and dc118 at 21:03 ET sat silent for weeks.
const TICK_WINDOW_MIN = 30;
...
async function handleTick() {
  ...
  for (const t of trustRows) {
    ...
    const rawOverride = t.tomorrow_override as { sendTime?: string; skipDay?: boolean; validUntil?: string } | null;
    // tomorrow_override is supposed to last one day. Without an expiry it
    // gets stuck: a `skipDay=true` blocks every subsequent tick (because
    // the planner that would clear it is the one being blocked). Treat
    // missing `validUntil` (legacy rows) as already-expired so they self-
    // heal on the next tick. Fire-and-forget the cleanup write — losing
    // it is fine, it'll just retry next tick.
    const overrideExpired = !!rawOverride && (!rawOverride.validUntil || Date.parse(rawOverride.validUntil) <= nowMs);
    if (overrideExpired) {
      void admin
        .from("trust_levels")
        .update({ tomorrow_override: null, updated_at: new Date().toISOString() })
        .eq("user_id", t.user_id);
    }
    const override = overrideExpired ? null : rawOverride;
