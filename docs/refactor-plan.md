...
> Answer: **Skip the external queue. Use parallel-cron with self-balancing batches.** Cron fires every minute (`* * * * *` in `vercel.json`); each tick processes up to 50 gmail-connected users whose last Watcher run is older than 40 min, sorted oldest-first. At ≤2k users this means full coverage in ≤40 min and idle ticks the rest of the cycle; at ≥2k users it runs flat-out and the cycle stretches naturally. Watcher is I/O-bound (Gmail history-id check + optional Claude classify only when there are new messages) so 50 in-flight async calls fit comfortably in a 60s function. The Planner side is naturally throttled by users' staggered `preferred_send_time` and now also dedups against `agent_runs` to prevent double-fires when cron drift straddles a tick boundary. Both sides parallelized via `Promise.allSettled`. Future scale-out path (~2k+ users or sub-40-min freshness): swap the bodies of `dispatchWatcher` / `dispatchPlanner` in `src/services/queue/dispatch.ts` from direct calls to `qstash.publishJSON(...)` — call sites in the cron tick don't change. Vendor-swap insurance was the whole point of the abstraction. Cost today: $0. Cost at 1k users: $0. No vendor account, no env vars, no new dependency.
